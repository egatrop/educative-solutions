...
internal object TaskScheduling {
    private fun isSchedulingPossible(tasks: Int, prerequisites: List<List<Int>>): Boolean {
        ...
        return scheduled.size == tasks
    }

    @JvmStatic
    fun main(args: Array<String>) {
        var result = isSchedulingPossible(3, listOf(listOf(0, 1), listOf(1, 2)))
        println("Tasks execution possible: $result") // true
        result = isSchedulingPossible(3, listOf(listOf(0, 1), listOf(1, 2), listOf(2, 0)))
        println("Tasks execution possible: $result") // false
